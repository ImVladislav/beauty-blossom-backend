...
const { inProgressDesk } = require('../models/inProgressDesk')
...
const getAll = async (req, res) => {
    ...
    const result = await inProgressDesk.find({owner}, "-createdAt -updatedAt").populate("owner", "name email");
...
const getById = async (req, res) => {
    ...
    const result = await inProgressDesk.findById(id);
    if (!result) {
...
const add = async (req, res) => {
    ...
    const result = await inProgressDesk.create({ ...req.body, owner });
...
const updateById = async (req, res) => {
  ...
  const { amount } = req.body;
    ...
    const currentItem = await inProgressDesk.findById(id);
...
const updateCheked = async (req, res) => {
    const { id } = req.params;
    const result = await inProgressDesk.findByIdAndUpdate(id, req.body, {new: true});
    if (!result) {
        throw HttpError(404, "Not found");
    }
    res.json(result);
...
const deleteById = async (req, res) => {
    const { id } = req.params;
    const result = await inProgressDesk.findByIdAndRemove(id);
    if (!result) {
        throw HttpError(404, "Not found");
    }
    res.json({
