...
const getById = async (req, res) => {
    ...
    const result = await Wood.findById(id);
    if (!result) {
...
const add = async (req, res) => {
    const {_id: owner} = req.user;
    const result = await Wood.create({ ...req.body, owner });
...
const updateById = async (req, res) => {
    const { id } = req.params;
    const result = await Wood.findByIdAndUpdate(id, req.body, {new: true});
    if (!result) {
        throw HttpError(404, "Not found");
    }
    res.json(result);
...
const deleteById = async (req, res) => {
    const { id } = req.params;
    const result = await Wood.findByIdAndRemove(id);
    if (!result) {
        throw HttpError(404, "Not found");
    }
    res.json({
